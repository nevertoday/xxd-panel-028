<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 028 project banner" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-paper-miniature-logic)
...
028 reverses that logic:

```text
lock identity / silhouette / pose / relation → simplify into orthographic masses and restrained tiers → place on one source-earned paper base → establish scale through edge crop, light shadow, and very few details → reduce and soften a source-derived palette → finish with fine ink, flat colour, and subtle paper grain
```
...
- **Restrained scale cues:** mass contrast, edge crop, light shadow, and very few figures, railings, trees, tracks, or objects establish scale without a second focus.
- **Orthographic isometric:** the camera is clean and masses readable, without photographic perspective, impossible space, busy cities, or game levels.
- **Source-derived palette:** identifying colours are reduced, purified, softened, and kept faithful to the source's warm/cool character—never a fixed pastel card.
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 028 isometric paper miniature below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 028 isometric paper miniature right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `028.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore a different miniature scale, base shape, mass hierarchy, source-derived palette, and type relation.
...
## Copy becomes a measure line for the miniature
...
Automatic copy derives one short title from source-supported theme, place, action, state, relation, emotion, or scale tension. Add only a very small number of source-grounded indexes, short lines, or micro-notes when genuinely useful. It uses only the target language by default; bilingual output appears only when explicitly requested.

Places, years, dates, numbers, and provenance must be user-supplied or reliably established; none are invented for atmosphere, and no English translation is silently appended. Exact user copy remains verbatim. Type aligns with the whitespace edge, paper base, horizontal axis, or isometric structure.
...
The image model creates the source-bound isometric masses, paper base, scale cues, source-derived limited palette, fine ink, flat colour, light shadow, subtle paper grain, and thin editorial type. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
Claude Code users may link the same directory to `~/.claude/skills/xxd-panel-028`. Restart the agent session after installation.
...
- [Chinese full prompt](references/xxd-panel-028-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-028-prompt.en.md)
- [Original style brief](references/028-source.md)
...
**Do not place the photograph inside a toy town; place its unique mass, colour, and solitude on paper.**
